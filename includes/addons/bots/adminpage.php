...
function bots_admin_page ( $array = array() )
{
	...
	elseif ($array['pagetag'] === 'edit-bot' && isset($_GET['botname']))
	{
		...
		$getstats = $pdo->query("SELECT * FROM " . DB_STATS . " WHERE player = '{$botname}'");
		$usestats = $getstats->fetch(PDO::FETCH_ASSOC);
		$botpot   = $usestats['winpot'];

		$ptblQ = $pdo->query("SELECT p1name, p1pot, p2name, p2pot, p3name, p3pot, p4name, p4pot, p5name, p5pot, p6name, p6pot, p7name, p7pot, p8name, p8pot, p9name, p9pot, p10name, p10pot FROM " . DB_POKER . " WHERE (p1name = '{$botname}' OR p2name = '{$botname}' OR p3name = '{$botname}' OR p4name = '{$botname}' OR p5name = '{$botname}' OR p6name = '{$botname}' OR p7name = '{$botname}' OR p8name = '{$botname}' OR p9name = '{$botname}' OR p10name = '{$botname}')");
		if ($ptblQ->rowCount() === 1)
		{
			$ptblF = $ptblQ->fetch(PDO::FETCH_ASSOC);
			$x     = 0;

			for ($i = 1; $i < 11; $i++)
			{ 
				if ($ptblF["p{$i}name"] === $botname)
				{
					$x = (int) $ptblF["p{$i}pot"];
					break;
				}
