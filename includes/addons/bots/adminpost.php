...
function bots_admin_create ( $array = array() )
{
    ...
    {
        $pdo->exec("ALTER TABLE `" . DB_POKER . "` ADD `botgod` TINYINT NULL AFTER `dealer`;");
    }
	...
	if ($action === 'createbot' && isset($_POST['cbotname'], $_POST['ctable'], $_POST['cseatnumber'], $_POST['cbotpot']))
	{
	    ...
	    if ($GLOBALS['bad_msg'] == false)
	    {
	        $zq = $pdo->prepare("SELECT gameID FROM " . DB_POKER . " WHERE gameID = $tableid and (p1name = '$botname' OR p2name = '$botname' OR p3name = '$botname' OR p4name = '$botname' OR p5name = '$botname' OR p6name = '$botname' OR p7name = '$botname' OR p8name = '$botname' OR p9name = '$botname' OR p10name = '$botname')");
	        $zq->execute();
