...
function incpoker_change_botgod ( $array = array() )
{
    ...
    if ($nxtbp !== false)
    {
        $pdo->query("UPDATE " . DB_POKER . " SET botgod = {$nxtbp} WHERE gameID = {$gameID}");
    }
    else
    {
        $set = array();
        ...
        $pdo->query("UPDATE " . DB_POKER . " SET move = 0, dealer = 0, botgod = 0, hand = -1, pot = '0', bet = '0', lastbet = '', $setstring WHERE gameID = " . $gameID);
    }
}
$addons->add_hook(array(
    'page'     => 'includes/inc_poker.php',
