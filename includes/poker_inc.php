...
function money($val)
{
    ...
    if (is_numeric($val))
    {
        ...
        if ($val > 1000000000)
        {
            $money = MONEY_PREFIX . number_format(($val / 1000000000) , 0, MONEY_THOUSA, MONEY_DECIMA) . ' B';
        }
        elseif ($val > 100000000)
        {
            $money = MONEY_PREFIX . number_format(($val / 1000000) , 0, MONEY_THOUSA, MONEY_DECIMA) . ' M';
        }
        elseif ($val > 1000000)
        {
            $money = MONEY_PREFIX . number_format(($val / 1000000) , 1, MONEY_THOUSA, MONEY_DECIMA) . ' M';
        }
        elseif ($smallbetfunc == 1)
        {
            $money = MONEY_PREFIX . number_format($val, 2, MONEY_THOUSA, MONEY_DECIMA);
        }
        else
        {
            if ($pos = strpos($val, '.'))
            {
                $len = strlen($val);
                $dec = $len - ($pos + 1);
                $money = MONEY_PREFIX . number_format($val, $dec, MONEY_THOUSA, MONEY_DECIMA);
            }
            else
                $money = MONEY_PREFIX . number_format($val, 0, MONEY_THOUSA, MONEY_DECIMA);
        }
    }
    elseif ($val == 'FOLD')
    {
...
function money_small($val)
{
    ...
    if ($val > 999999999)
    {
        $money = MONEY_PREFIX . number_format(($val / 1000000000) , 1, MONEY_DECIMA, MONEY_THOUSA) . 'B';
    }
    elseif ($val > 99999999)
    {
        $money = MONEY_PREFIX . number_format(($val / 1000000) , 0, MONEY_DECIMA, MONEY_THOUSA) . 'M';
    }
    elseif ($val > 999999)
    {
        $money = MONEY_PREFIX . number_format(($val / 1000000) , 1, MONEY_DECIMA, MONEY_THOUSA) . 'M';
    }
    elseif ($val > 999)
    {
        if (($val % 1000) == 0)
        {
            $money = MONEY_PREFIX . number_format(($val / 1000) , 0, MONEY_DECIMA, MONEY_THOUSA) . 'K';
        }
        else
        {
            $money = MONEY_PREFIX . number_format(($val / 1000) , 1, MONEY_DECIMA, MONEY_THOUSA) . 'K';
        }
    }
    elseif ($smallbetfunc == 1)
    {
        $money = MONEY_PREFIX . number_format($val, 2, MONEY_DECIMA, MONEY_THOUSA);
    }
    else
    {
        if ($pos = strpos($val, '.'))
        {
            $len = strlen($val);
            $dec = $len - ($pos + 1);
            $money = MONEY_PREFIX . number_format($val, $dec, MONEY_DECIMA, MONEY_THOUSA);
        }
        else
            $money = MONEY_PREFIX . number_format($val, 0, MONEY_DECIMA, MONEY_THOUSA);
    }
